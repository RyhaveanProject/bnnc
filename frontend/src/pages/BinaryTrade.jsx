...
import api, { TRADING_PAIRS, COIN_NAMES, fmtMoney } from "../lib/api";
...
export default function BinaryTrade() {
  ...
  const usdtBalance = (user && user.balances) ? (user.balances.USDT || 0) : 0;
  const selectedDuration = DURATIONS.find(d => d.seconds === duration);
  const estimatedProfit = (amount && selectedDuration) ? (amount * selectedDuration.profit / 100) : 0;
  const circumference = 2 * Math.PI * 80;
  const progressPct = activeTrade ? Math.max(0, Math.round((timeLeft / activeTrade.duration) * 100)) : 0;

  const resetAll = () => {
    clearInterval(timerRef.current);
    setStep("select"); setAmount(null); setDuration(null); setDirection(null);
    setActiveTrade(null); setTimeLeft(0); setResult(null); setErr("");
  };

  const completeTrade = useCallback((tradeId) => {
    setBusy(true);
    api.post("/binary-trade/complete/" + tradeId)
      .then(res => {
        setResult({ win: res.data.result === "win", profit: res.data.profit, payout: res.data.payout });
        return refresh();
      })
      .then(() => { setStep("result"); setBusy(false); })
      .catch(e => {
        ...
        if (detail.includes("not yet expired")) {
          ...
          setErr(detail || "Error completing trade.");
          setStep("result");
          setResult({ win: false, profit: 0, payout: 0 });
          setBusy(false);
        }
      });
  }, [refresh]);
  ...
  const placeTrade = () => {
    if (!symbol || !amount || !duration || !direction) return;
    setErr(""); setBusy(true);
    api.post("/binary-trade/place", { symbol, amount_usd: amount, duration, direction })
      .then(res => { setActiveTrade(res.data.trade); setTimeLeft(duration); setStep("active"); return refresh(); })
      .then(() => setBusy(false))
      .catch(e => {
        setErr((e && e.response && e.response.data ? e.response.data.detail : null) || "Failed to place trade.");
        setBusy(false);
      });
  };
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) { clearInterval(timerRef.current); completeTrade(activeTrade.id); return 0; }
        return prev - 1;
      });
    }, 1000);
    return () => clearInterval(timerRef.current);
  }, [step, activeTrade, completeTrade]);

  return (
    <div data-testid="binary-trade-page" style={{ maxWidth: 900, margin: "0 auto", padding: 24 }} className="container-pad">
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        <h1 style={{ margin: 0, fontSize: 24 }}>Trading</h1>
        <span className="pill approved" style={{ fontSize: 11 }}>Binary Options</span>
        <div style={{ marginLeft: "auto", textAlign: "right" }}>
          <div className="text-dim" style={{ fontSize: 11 }}>Available Balance</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: "var(--color-accent)" }}>${fmt(usdtBalance)} USDT</div>
        </div>
      </div>

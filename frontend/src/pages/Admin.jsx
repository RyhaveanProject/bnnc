...
function BalanceModal({ user, onClose, onUpdated }) {
  ...
  const currentBal = useMemo(
    () => Number(user?.balances?.[currency] || 0),
    [user, currency]
  );
  ...
  const submit = async () => {
    ...
    if (mode === "set" && amt < 0) {
      setErr("Balance cannot be negative");
      return;
    }
    if ((mode === "add" || mode === "subtract") && amt <= 0) {
    ...
    try {
      if (mode === "set") {
        const { data } = await api.post("/admin/users/balance/set", {
          user_id: user.id,
          currency,
          amount: amt,
        });
    ...
    >
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ margin: 0 }}>Manage Balance</h3>
          <button className="btn btn-ghost btn-sm" onClick={onClose} data-testid="balance-modal-close">
            ✕
          </button>
        </div>

        <div className="text-dim" style={{ fontSize: 13, marginBottom: 12 }}>
          User: <b style={{ color: "var(--text)" }}>{user.username}</b> ({user.email})
        </div>
        ...
        <div
          ...
          data-testid="bal-current"
        >
          Current {currency} balance:{" "}
          <b style={{ color: "var(--text)" }}>{fmtMoney(currentBal, 6)}</b>
        </div>
        ...
        <input
          ...
          placeholder="0.00"
          data-testid="bal-amount"
          autoFocus
        />

        {err && (
          <div className="text-red" style={{ marginTop: 10, fontSize: 13 }} data-testid="bal-err">
            {err}
          </div>
        )}

        <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
          <button className="btn btn-ghost" onClick={onClose} disabled={busy} style={{ flex: 1 }}>
            Cancel
          </button>
          <button
...
function Users() {
  ...
  useEffect(() => {
    load();
    const t = setInterval(load, 8000); // realtime-ish refresh
    return () => clearInterval(t);
  }, []);
  ...
  const onUpdated = (updatedUser) => {
    ...
    setMsg(`Balance updated for ${updatedUser.username}`);
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="tbl">
          ...
          <tbody>
              ...
              <tr key={u.id}>
                ...
                <td style={{ fontSize: 11 }}>
                  {Object.entries(u.balances || {})
                    .filter(([, v]) => v > 0)
                    .map(([k, v]) => `${k}:${fmtMoney(v, 4)}`)
                    .join(" ") || "—"}
                </td>
                <td>
                  {u.banned ? (
                    <span className="pill rejected">Banned</span>
                  ) : (
                  ...
                  {u.role !== "admin" && (
                    <>
                      <button
                        ...
                        data-testid={`set-balance-${u.id}`}
                      >
                        Set Balance
                      </button>{" "}
                      <button
                        className={`btn btn-sm ${u.banned ? "btn-green" : "btn-red"}`}
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="text-dim" style={{ textAlign: "center", padding: 24 }}>
                  No users found
                </td>
              </tr>
            )}
...
function Deposits() {
  ...
  const act = async (id, ok) => {
    try {
      ...
      setMsg(ok ? "Deposit approved" : "Deposit rejected");
  ...
  return (
    <div className="panel" data-testid="admin-deposits">
          ...
          <tbody>
            {rows.map((d) => (
              <tr key={d.id}>
                ...
                <td>
                  <div>{d.username}</div>
                  <div className="text-dim" style={{ fontSize: 11 }}>{d.email}</div>
                </td>
                <td>{d.currency}</td>
                <td>{d.amount}</td>
                <td style={{ fontSize: 11, maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis" }} title={d.wallet_address}>
                  {d.wallet_address}
                </td>
                <td>
                  {d.receipt_uploaded ? (
                    <button
                      className="btn btn-ghost btn-sm"
                      onClick={() => setViewingReceipt(d.id)}
                      data-testid={`view-receipt-${d.id}`}
                    >
                      View
                    </button>
                  ) : (
                ...
                <td>
                  {d.status === "pending" && (
                    <>
                      <button className="btn btn-green btn-sm" onClick={() => act(d.id, true)} data-testid={`apr-${d.id}`}>
                        Confirm
                      </button>{" "}
                      <button className="btn btn-red btn-sm" onClick={() => act(d.id, false)} data-testid={`rej-${d.id}`}>
                        Cancel
                      </button>
                    </>
                  )}
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={8} className="text-dim" style={{ textAlign: "center", padding: 24 }}>
                  No deposits yet
                </td>
              </tr>
            )}
      ...
      {viewingReceipt && (
        <ReceiptViewer depositId={viewingReceipt} onClose={() => setViewingReceipt(null)} />
      )}
    </div>
  );
}
...
function Withdrawals() {
  const [rows, setRows] = useState([]);
  useEffect(() => {
    api.get("/admin/withdrawals").then((r) => setRows(r.data)).catch(() => {});
  }, []);
  return (
...
function Wallets() {
  ...
  const startEdit = (w) => {
    setEditing(w.currency);
    setAddress(w.address);
    setNetwork(w.network || "");
    setQrFile(w.qr_image_b64 || "");
    setMsg("");
  };
  const onFile = (e) => {
        ...
        return (
          <div key={c} className="panel" style={{ padding: 16, marginBottom: 12, background: "#0f1319" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              {!isEditing && (
                <button className="btn btn-ghost btn-sm" onClick={() => startEdit(w)} data-testid={`edit-wallet-${c}`}>
                  Edit
                </button>
              )}
            </div>
            {!isEditing ? (
              <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 13, wordBreak: "break-all" }}>
                    {w.address || <span className="text-dim">not set</span>}
                  </div>
                </div>
              </div>
            ) : (
              <div>
                ...
                {qrFile && (
                  <img src={qrFile} alt="qr" style={{ maxWidth: 140, marginTop: 8, borderRadius: 6, background: "#fff", padding: 4 }} />
                )}

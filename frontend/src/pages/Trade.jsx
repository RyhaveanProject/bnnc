...
export default function Trade() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (!amt || amt <= 0) throw new Error(t(\"trade.invalid_amount\"));
      const { data: res } = await api.post(\"/trade/execute\", { symbol, side, amount: amt });
      if (side === \"buy\") {
        setMsg(t(\"trade.bought_msg\", { qty: res.trade.executed.qty.toFixed(6), symbol, price: fmtMoney(res.trade.price) }));
      } else {
        setMsg(t(\"trade.sold_msg\", { received: res.trade.executed.received.toFixed(2), price: fmtMoney(res.trade.price) }));
      }
      setAmount(\"\");
      await refresh();
  ...
  return (
    <div data-testid=\"trade-page\" style={{maxWidth:1280, margin:\"0 auto\", padding:24}} className=\"container-pad\">
      <div className=\"panel\" style={{padding:16, marginBottom:16, display:\"flex\", gap:24, alignItems:\"center\", flexWrap:\"wrap\"}} data-testid=\"portfolio-summary\">
        <div>
          <div className=\"text-dim\" style={{fontSize:12}}>{t(\"trade.estimated_balance\")}</div>
        ...
        <div style={{display:\"flex\", gap:16, flexWrap:\"wrap\"}}>
          ...
          {portfolio.rows.length === 0 && (
            <div className=\"text-dim\" style={{fontSize:13}}>{t(\"trade.no_holdings\")}</div>
          )}
        </div>
      </div>
      ...
      <div style={{display:\"grid\", gridTemplateColumns:\"260px 1fr 360px\", gap:16}} className=\"trade-grid\">
        <div className=\"panel\" style={{padding:12, maxHeight:600, overflowY:\"auto\"}}>
          <div style={{fontWeight:600, padding:\"6px 8px\"}}>{t(\"trade.pairs\")}</div>
        ...
        <div className=\"panel\" style={{padding:20}}>
          ...
          <div style={{display:\"grid\", gridTemplateColumns:\"repeat(3, 1fr)\", gap:12, marginTop:8}}>
            ...
            <Stat label={t(\"trade.stat_market_cap\")} value={`$${fmtMoney(coin?.marketCap ?? 0, 0)}`} />
            <Stat label={t(\"trade.stat_price\")} value={`$${fmtMoney(price)}`} />
          </div>
        </div>

        <div className=\"panel\" style={{padding:20}}>
          <div style={{display:\"flex\", gap:8, marginBottom:16}}>
            <button onClick={()=>setSide(\"buy\")} data-testid=\"side-buy\" className={`btn ${side===\"buy\"?\"btn-green\":\"btn-ghost\"}`} style={{flex:1}}>{t(\"trade.buy\")}</button>
            <button onClick={()=>setSide(\"sell\")} data-testid=\"side-sell\" className={`btn ${side===\"sell\"?\"btn-red\":\"btn-ghost\"}`} style={{flex:1}}>{t(\"trade.sell\")}</button>
          </div>
          <div className=\"text-dim\" style={{fontSize:13, marginBottom:10}}>
            {t(\"trade.balance\")}: <span style={{color:\"#fff\"}}>{fmtMoney(usdtBal)} USDT</span> · <span style={{color:\"#fff\"}}>{fmtMoney(coinBal, 6)} {symbol}</span>
          </div>
          <form onSubmit={submit}>
            <label className=\"lbl\">{t(\"trade.price_market\")}</label>
            <input className=\"input\" value={`$${fmtMoney(price)}`} disabled />
            <div style={{height:10}}/>
            <label className=\"lbl\">{side === \"buy\" ? t(\"trade.amount_to_spend\") : t(\"trade.amount_to_sell\", { symbol })}</label>
            ...
            <button type=\"submit\" disabled={busy || !user} className={`btn ${side===\"buy\"?\"btn-green\":\"btn-red\"}`} style={{width:\"100%\", marginTop:16}} data-testid=\"trade-submit\">
              {busy ? <span className=\"spinner\"/> : `${side===\"buy\"?t(\"trade.buy\"):t(\"trade.sell\")} ${symbol}`}
            </button>

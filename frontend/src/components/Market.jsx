...
export function PriceTicker() {
  ...
  if (!data.length) return <div className=\"ticker\"><span className=\"text-dim\">{t(\"landing.loading_markets\")}</span></div>;
  return (
...
export function MarketTable({ compact = false, onSelect }) {
  ...
  return (
    <div className=\"panel\" data-testid=\"market-table\">
      <div style={{ padding: \"16px 20px\", borderBottom: \"1px solid var(--border)\", fontWeight: 600 }}>{t(\"landing.live_markets\")}</div>
      <div style={{overflowX:\"auto\"}}>
      <table className=\"tbl\">
        <thead>
          <tr>
            <th>{t(\"markets.pair\")}</th><th>{t(\"markets.price\")}</th><th>{t(\"markets.change_24h\")}</th>{!compact && <th className=\"hide-mobile\">{t(\"markets.volume\")}</th>}<th>{t(\"markets.chart\")}</th>
            {onSelect && <th></th>}
          </tr>
        </thead>
        <tbody>
          {data.map(d => (
            <tr key={d.symbol} className={flash[d.symbol]===\"up\"?\"flash-up\":flash[d.symbol]===\"down\"?\"flash-down\":\"\"}>
              ...
              {onSelect && <td><button className=\"btn btn-primary btn-sm\" data-testid={`trade-${d.symbol}-btn`} onClick={() => onSelect(d.symbol)}>{t(\"markets.trade_btn\")}</button></td>}
            </tr>
          ))}
        </tbody>
      </table>
